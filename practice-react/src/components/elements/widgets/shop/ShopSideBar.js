...
import ShopSideCategory from './ShopSideCategory'

export default function ShopSideBar({ category, setCategory }) {
    ...
    const categoryList = categoryData.map(item => {
        return (
            <ShopSideCategory key={item.id} name={item.name} category={category} setCategory={setCategory} />
        )
    })
